...
namespace byh_api.Models
{
    ...
    [ApiController]
    public class BodyOilsController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;

        public BodyOilsController(IConfiguration configuration, IWebHostEnvironment env)
        {
        ...
        public JsonResult Get()
        {
            ...
            try
            {
                string query = @"SELECT * from dbo.BodyOils";
        ...
        [HttpPost]
        public JsonResult Post(BodyOils prod)
        {
            Response response = new Response();

            try
            {
                string query = @"INSERT INTO dbo.BodyOils VALUES(@ProductName, @ProductTypeId, 
        ...
        [HttpPut("UpdateBodyOil/{Id}")]
        public JsonResult UpdateBodyOil(BodyOils prod, int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.BodyOils SET ProductName = @ProductName, ProductTypeId = @ProductTypeId, 
        ...
        [HttpPut("DelBodyOil/{Id}")]
        public JsonResult DelBodyOils(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.BodyOils SET IsDeleted = 1
        ...
        [HttpPut("RevBodyOil/{Id}")]
        public JsonResult RevBodyOil(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.BodyOils SET IsDeleted = 0
        ...
        public JsonResult SaveFile()
        {
            ...
            try
            {
                ...
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    var physicalPath = Path.Combine(_env.ContentRootPath, "Photos/BodyOils", filename);
