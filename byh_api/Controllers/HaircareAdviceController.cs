...
namespace byh_api.Controllers
{
    ...
    public class HaircareAdviceController : ControllerBase
    {
        ...
        [HttpPost]
        public JsonResult Post(HaircareAdvice question)
        {
            Response response = new Response();

            try
            {
                string query = @"INSERT INTO dbo.HaircareAdvice (UserId, UserGender, UserAge, PregnantOrFeeding, HairType, MainIssueId, 
                                MainIssue, MainSolution, HaircareRoutineId, Step1, Step2, Step3, Step4, Step5, Step6, Step7, Step8, Step9, Step10, 
                                DiagnDate, IsDeleted)
                                VALUES (@UserId, @UserGender, @UserAge, @PregnantOrFeeding, @HairType, @MainIssueId, 
                                    (SELECT HairProblem FROM dbo.HairProblems WHERE Id = @MainIssueId), 
                                    (
                                        SELECT STRING_AGG(CONCAT(Solution, ' - ', Description), ', ') AS CombinedSolutions
                                    ...
                                    (
                                        CASE
                                            ...
                                            WHEN (SELECT Step1 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Złuszczanie' THEN
                                                (SELECT TOP 1 CONCAT('Złuszczanie: ', ProductName, ' - ', Frequency) FROM dbo.HairPeelings WHERE HairType = @HairType AND ProductType IN ('Peeling'))
                                            ELSE '' 
                                        END
                                    ),
                                    (
                                        CASE
                                            WHEN (SELECT Step2 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Olejowanie' THEN
                                                (SELECT TOP 1 CONCAT('Olejowanie: ', ProductName, ' - ', Frequency) FROM dbo.HairOils WHERE HairType = @HairType AND ProductType IN ('Olejowanie'))
                                            ELSE '' 
                                        END
                                    ),
                                    (
                                        CASE
                                            WHEN (SELECT Step3 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Maska' THEN
                                                (SELECT TOP 1 CONCAT('Maska: ', ProductName, ' - ', Frequency) FROM dbo.HairMasks WHERE HairType = @HairType AND ProductType IN ('Maska'))   
                                                    ...
                                                    CASE
                                                        WHEN EXISTS (
                                                            ...
                                                            WHERE HairType = @HairType AND ProductType IN (
                                                                SELECT Solution
                                                                FROM dbo.DealingHairProblems
                                                                WHERE HairProblem IN (SELECT HairProblem FROM dbo.HairProblems WHERE Id IN (@MainIssueId)) AND IsDeleted = 0
                                                            )
                                                        )
                                                        THEN
                                                            (
                                                        ...
                                                        ELSE (SELECT Step3 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Maska' THEN
                                    ...
                                    (
                                        CASE
                                            WHEN (SELECT Step4 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Mycie' THEN
                                                (SELECT TOP 1 CONCAT('Mycie: ', ProductName, ' - ', Frequency) FROM dbo.Shampoos WHERE HairType = @HairType AND ProductType IN ('Szampon'))
                                    ...
                                    (
                                        CASE
                                            ...
                                            WHEN (SELECT Step6 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Krem' THEN
                                                (SELECT TOP 1 CONCAT('Krem: ', ProductName, ' - ', Frequency) FROM dbo.HairCreams WHERE HairType = @HairType AND ProductType IN ('Krem'))
                                            ELSE '' 
                                        END
                                    ),
                                    (
                                        CASE
                                            WHEN (SELECT Step7 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Termoochrona' THEN
                                                (SELECT TOP 1 CONCAT('Termoochrona: ', ProductName, ' - ', Frequency) FROM dbo.HeatProtection WHERE HairType = @HairType AND ProductType IN ('Termoochrona')) 
                                    ...
                                    (
                                        CASE
                                            ...
                                            WHEN (SELECT Step8 FROM dbo.HaircareSteps WHERE HairType = @HairType) = 'Olejek ochronny' THEN
                                                (SELECT TOP 1 CONCAT('Olejek ochronny: ', ProductName, ' - ', Frequency) FROM dbo.HairOilsProtection WHERE HairType = @HairType AND ProductType IN ('Olejek ochronny'))
                                            ELSE '' 
                                        END
                                    ),
                                    (
