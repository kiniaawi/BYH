...
namespace byh_api.Controllers
{
    ...
    [ApiController]
    public class BodyExfoliantsController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;

        public BodyExfoliantsController(IConfiguration configuration, IWebHostEnvironment env)
        {
        ...
        public JsonResult Get()
        {
            ...
            try
            {
                string query = @"SELECT * from dbo.BodyExfoliants";
        ...
        [HttpPost]
        public JsonResult Post(BodyExfoliants prod)
        {
            Response response = new Response();

            try
            {
                string query = @"INSERT INTO dbo.BodyExfoliants VALUES(@ProductName, @ProductTypeId, 
        ...
        [HttpPut("UpdateBodyExfoliant/{Id}")]
        public JsonResult UpdateBodyExfoliant(BodyExfoliants prod, int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.BodyExfoliants SET ProductName = @ProductName, ProductTypeId = @ProductTypeId, 
        ...
        [HttpPut("DelBodyExfoliant/{Id}")]
        public JsonResult DelBodyExfoliant(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.BodyExfoliants SET IsDeleted = 1
        ...
        [HttpPut("RevBodyExfoliant/{Id}")]
        public JsonResult RevBodyExfoliant(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.BodyExfoliants SET IsDeleted = 0
        ...
        public JsonResult SaveFile()
        {
            ...
            try
            {
                ...
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    var physicalPath = Path.Combine(_env.ContentRootPath, "Photos/BodyExfoliants", filename);
