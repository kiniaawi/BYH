...
namespace byh_api.Controllers
{
    ...
    [ApiController]
    public class HairProblemsController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;

        public HairProblemsController(IConfiguration configuration, IWebHostEnvironment env)
        {
        ...
        public JsonResult Get()
        {
            ...
            try
            {
                string query = @"SELECT * from dbo.HairProblems";
        ...
        [HttpPost]
        public JsonResult Post(HairProblems prod)
        {
            Response response = new Response();
        ...
        [HttpPut("UpdateHairProblem/{Id}")]
        public JsonResult UpdateHairProblem(HairProblems prod, int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HairProblems SET HairProblem = @HairProblem, ImageURL = @ImageURL
                            WHERE Id = @Id";
        ...
        [HttpPut("DelHairProblem/{Id}")]
        public JsonResult DelHairProblem(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HairProblems SET IsDeleted = 1
        ...
        [HttpPut("RevHairProblem/{Id}")]
        public JsonResult RevHairProblem(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.HairProblems SET IsDeleted = 0
        ...
        public JsonResult SaveFile()
        {
            ...
            try
            {
                ...
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    var physicalPath = Path.Combine(_env.ContentRootPath, "Photos/HairProblems", filename);
