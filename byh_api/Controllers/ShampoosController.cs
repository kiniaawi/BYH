...
namespace byh_api.Controllers
{
    ...
    [ApiController]
    public class ShampoosController : ControllerBase
    {
        private readonly IConfiguration _configuration;
        private readonly IWebHostEnvironment _env;

        public ShampoosController(IConfiguration configuration, IWebHostEnvironment env)
        {
        ...
        public JsonResult Get()
        {
            ...
            try
            {
                string query = @"SELECT * from dbo.Shampoos";
        ...
        [HttpPost]
        public JsonResult Post(Shampoos prod)
        {
            Response response = new Response();

            try
            {
                string query = @"INSERT INTO dbo.Shampoos VALUES(@ProductName, @ProductTypeId, 
        ...
        [HttpPut("UpdateShampoo/{Id}")]
        public JsonResult UpdateShampoo(Shampoos prod, int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.Shampoos SET ProductName = @ProductName, ProductTypeId = @ProductTypeId, 
        ...
        [HttpPut("DelShampoo/{Id}")]
        public JsonResult DelShampoo(int Id)
        {
            Response response = new Response();

            try
            {
                string query = @"UPDATE dbo.Shampoos SET IsDeleted = 1
                ...
                using (SqlConnection myConn = new SqlConnection(sqlDataSource))
                {
                    ...
                    response.StatusCode = 200;
                    response.StatusMessage = "Shampoos Deleted Successfully";
                    HttpContext.Response.StatusCode = response.StatusCode;
                    response.Data = table;
                }
            }
            catch (Exception ex)
            {
                ...
                response.StatusCode = 100;
                response.StatusMessage = "Failed to Delete Shampoos";
                HttpContext.Response.StatusCode = response.StatusCode;
            }
        ...
        public JsonResult RevShampoo(int Id)
        {
            ...
            try
            {
                string query = @"UPDATE dbo.Shampoos SET IsDeleted = 0
                ...
                using (SqlConnection myConn = new SqlConnection(sqlDataSource))
                {
                    ...
                    response.StatusCode = 200;
                    response.StatusMessage = "Shampoos Reverted Successfully";
                    HttpContext.Response.StatusCode = response.StatusCode;
                    response.Data = table;
                }
            }
            catch (Exception ex)
            {
                ...
                response.StatusCode = 100;
                response.StatusMessage = "Failed to Revert Shampoos";
                HttpContext.Response.StatusCode = response.StatusCode;
            }
        ...
        public JsonResult SaveFile()
        {
            ...
            try
            {
                ...
                if (postedFile != null && postedFile.Length > 0)
                {
                    ...
                    var physicalPath = Path.Combine(_env.ContentRootPath, "Photos/Shampoos", filename);
