...
const HairNailsSupplAdvice = ({ onChangeContent }) => {
  ...
  setCookie("currentPageCookie", "hair-nails-suppl-advice", { path: "/" });
  ...
  const fetchIssues = () => {
    axios
      .get(
        `/api/SupplHairNailsDiagnosticQuestions/GetHairNailsSupplAdvice/${userId}`
      )
      .then((response) => {
  ...
  const saveAsPDF = async () => {
    ...
    doc.save("Porada_Suplementacyjna_Wlosy_Paznokcie.pdf");
  };
  ...
  return (
    <Box flex={12} p={2} sx={{ height: "300vh" }}>
      ...
      <Typography variant="h5" sx={{ textAlign: "center", marginBottom: 3 }}>
        <b>Porada Suplementacyjna - Włosy i Paznokcie</b>
      </Typography>
...
export default HairNailsSupplAdvice;
