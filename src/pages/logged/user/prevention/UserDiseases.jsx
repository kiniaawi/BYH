...
const UserDiseases = ({ onChangeContent }) => {
  const [cookies, setCookie, removeCookie] = useCookies([
    "emailCookie",
    "currentPageCookie",
    "userIdCookie",
  ]);
  setCookie("currentPageCookie", "user-diseases", { path: "/" });
  ...
  const handleSubmitQuestions = () => {
    ...
    axios
      .post("/api/UserDiseasesAdvice", data)
      .then((response) => {
  ...
  const handleSeeAdvice = () => {
    clearTextArea();
    onChangeContent("user-diseases-advice");
    navigate("/user-diseases-advice");
  };
