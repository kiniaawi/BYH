...
const UserFaceSkincare = ({ onChangeContent }) => {
  const [cookies, setCookie, removeCookie] = useCookies([
    "emailCookie",
    "currentPageCookie",
    "userIdCookie",
  ]);
  setCookie("currentPageCookie", "user-face-skincare", { path: "/" });
  ...
  const [allFaceSkinIssues, setAllFaceSkinIssues] = useState([]);
  ...
  const fetchFaceSkinIssues = () => {
    axios
      .get("/api/SkinIssues/GetFaceSkinIssues")
      .then((response) => {
        console.log(response.data);
        console.log(response.data.Data);
        setAllFaceSkinIssues(response.data.Data);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const handleSubmitQuestions = () => {
    const data = {
      ...
      SecondIssueId: secondIssueId,
    };

    axios
      .post("/api/FaceSkincareAdvice/PostMorningFaceSkincareAdvice", data)
      .then((response) => {
  ...
  const handleSeeAdvice = () => {
    ...
    axios
      .post("/api/FaceSkincareAdvice/PostEveningFaceSkincareAdvice", data)
      .then((response) => {
        alert(response.data.StatusMessage);
        clearTextArea();
        onChangeContent("face-skincare-advice");
        navigate("/face-skincare-advice");
      })
      .catch((error) => {
        console.log(error);
      });
        ...
        <CardContent>
          <form onSubmit={handleSubmit}>
            ...
            <>
              ...
              <Stack direction="column" sx={{ marginTop: 3, marginLeft: 4 }}>
                <Typography sx={{ marginRight: 4, marginBottom: 2 }}>
                  Wybierz swoją drugą dolegliwość:
                </Typography>
                <Select
                  label="Druga Dolegliwość"
                  value={secondIssueId}
                  onChange={(e) => setSecondIssueId(e.target.value)}
                  required
                  sx={{ width: "200px" }}
                >
                  {allFaceSkinIssues
                    .filter((issue) => issue.Id !== mainIssueId)
                    .map((issue) => (
                      <MenuItem key={issue.Id} value={issue.Id}>
                        {issue.SkinIssue}
                      </MenuItem>
            ...
            {userAge !== "" && userAge < 12 ? (
              <Typography sx={{ marginLeft: 4, marginTop: 4 }} color="red">
                ...
                zaawansowana pielęgnacja skóry twarzy.
              </Typography>
            ) : (
              <Box></Box>
            )}
...
export default UserFaceSkincare;
