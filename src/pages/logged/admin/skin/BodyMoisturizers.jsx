...
import BodyMoisturizersTable from "./BodyMoisturizersTable";

const BodyMoisturizers = ({ onChangeContent }) => {
  const [cookies, setCookie, removeCookie] = useCookies([
    "emailCookie",
    "currentPageCookie",
  ]);
  setCookie("currentPageCookie", "body-moisturizers", { path: "/" });
  ...
  const handleAddProduct = () => {
    ...
    axios
      .post("api/BodyMoisturizers", data)
      .then((response) => {
  ...
  const handleAddFile = () => {
    ...
    axios
      .post("/api/BodyMoisturizers/SaveFile", formData)
      .then((response) => {
        alert(response);
      })
      .catch((error) => {
  ...
  return (
    <Box p={2} sx={{ height: "300vh" }}>
      ...
      <Typography variant="h5" sx={{ textAlign: "center", marginBottom: 3 }}>
        <b>Panel Produktów Nawilżających</b>
      </Typography>
      <Card>
        <CardHeader
          title={
      ...
      <Box>
        {" "}
        <BodyMoisturizersTable />
      </Box>
    </Box>
  );
};

export default BodyMoisturizers;
